...
3. Check for project-specific security conventions (a backend-engineering skill, app-level AGENTS.md, root AGENTS.md, existing auth/tenancy middleware patterns) and hold the code to those in addition to general best practices — a deviation from an established safe pattern is itself a finding.
